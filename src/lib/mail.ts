...
import { BRAND_FROM, BRAND_NAME } from "./brand";
...
async function sendSmtp(to: string, subject: string, text: string, html: string): Promise<void> {
  ...
  await transporter.sendMail({
    from: process.env.SMTP_FROM?.trim() || process.env.MAIL_FROM?.trim() || BRAND_FROM,
    to,
    subject,
    text,
    html,
...
async function sendResend(to: string, subject: string, text: string, html: string): Promise<void> {
  ...
  const from =
    process.env.MAIL_FROM?.trim() || process.env.RESEND_FROM?.trim() || BRAND_FROM;
...
export async function sendPasswordResetEmail(email: string, rawToken: string): Promise<void> {
  ...
  const html = `<p>Reset your ${BRAND_NAME} password:</p><p><a href="${resetUrl}">${resetUrl}</a></p><p>This link expires in one hour. If you did not request it, ignore this email.</p>`;
